define(["util"], function(util){
  ...
  Calendar.prototype = {
    ...
    refreshCal: function() {
      ...
      var lis = this.frame.querySelector(".calendar .days").querySelectorAll("li");
    ...
    addEvent: function() {
      ...
      //日历显示与隐藏
      eve.addListener(that.target, "click", function(){
      ...
      eve.addListener(frame, "click", function(){
        var e = event || window.event;
        if (e.stopPropagation) {
          e.stopPropagation();
        } else {
      ...
      eve.addListener(document.body, "click", function(){
        if (frame.style.visibility === "visible") {
          frame.style.visibility = "hidden";
        }      
      });
    ...
    changeDate: function() {
      ...
      if (this.isRange) {
        ...
        //选择月份最后一天并切换月份时，进行修正
        if (that.frame.querySelector(".calendar .days").querySelectorAll(".selected").length === 0) {
          var currMonth = that.frame.querySelector(".calendar .days").querySelectorAll(".currMonth");
    ...
    setDate: function(date) {
      ...
      this.year = year;
      that.frame.querySelector(".calendarHead").querySelector(".js-year").selectedIndex = year - startDate.getFullYear();
      that.frame.querySelector(".calendarHead").querySelector(".js-month").selectedIndex = this.month = newDate.getMonth();
